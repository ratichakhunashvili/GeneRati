...
// Words that say nothing about what an activity is, so they make poor keywords.
// Mostly the vocabulary of file names and design variants.
const NOISE = new Set([
  // File and design vocabulary
  'poster', 'template', 'design', 'final', 'copy', 'new', 'old', 'version', 'draft',
  'the', 'and', 'for', 'with', 'college', 'skillwill', 'event', 'png', 'jpg', 'jpeg',
  'red', 'blue', 'green', 'dark', 'light', 'black', 'white', 'purple', 'orange',
  'პოსტერი', 'შაბლონი', 'დიზაინი', 'ახალი', 'კოლეჯი', 'ღონისძიება',
  // Generic event words. These are the dangerous ones: a template keyed on
  // "night" would claim "Movie Night", "Quiz Night" and "Chess Night" alike,
  // and one keyed on "cup" would claim every tournament the college runs.
  'night', 'day', 'evening', 'morning', 'afternoon', 'cup', 'club', 'class',
  'session', 'week', 'weekend', 'annual', 'open', 'live', 'special',
  'ღამე', 'საღამო', 'დღე', 'კვირა', 'თასი',
]);

/**
 * Propose keywords for a template from its name.
 *
 * Two sources, in order of usefulness. First, if the name mentions a known
 * activity, return that activity's whole keyword list — which is the valuable
 * half, because it contributes the Georgian spellings for an English name and
 * vice versa, so "Football poster" also matches "ფეხბურთის ტურნირი". Second,
 * the meaningful words of the name itself, which is all that can be offered for
 * an activity the app does not know about.
 */
export function suggestKeywords(name) {
  ...
  for (const kind of KINDS) {
    if (kind.keywords.some((keyword) => text.includes(keyword))) {
      kind.keywords.forEach(add);
      break;
    }
  }

  // Split on anything that is not a letter or digit; \p{L} keeps Georgian.
  for (const word of text.split(/[^\p{L}\p{N}]+/u)) {
    if (word.length >= 3 && !NOISE.has(word) && !/^\d+$/.test(word)) add(word);
  }
