...
// These designs are authored against the A4 coordinate system and scaled onto
// the real page by documentShell. Every ISO paper size shares the same √2
// ratio, so one set of numbers works for any of them.
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .poster {
    ...
    /*
      Without this the poster is a flex item and shrinks to fit a window
      narrower than the page, silently rendering at the wrong size — which is
      exactly what a preview pane or a half-width browser window is.
    */
    flex: none;
    overflow: hidden;
  ...
  /*
    The design is laid out at ${WIDTH}×${HEIGHT} and scaled onto the page. The two
    axes are scaled independently because both sizes are rounded to whole
    pixels, and a single factor would leave a hairline gap down one edge.
  */
  .sheet {
    position: absolute;
    top: 0; left: 0;
    width: ${WIDTH}px;
    height: ${HEIGHT}px;
    transform: scale(${DESIGN_SCALE_X.toFixed(6)}, ${DESIGN_SCALE_Y.toFixed(6)});
